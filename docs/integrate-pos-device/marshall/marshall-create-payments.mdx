...
Once the SDK is running and the connection with the Nayax device is established (`onReady()` has been triggered), the device is ready to handle vending and cashless transactions.
...
<Note>
  **Multi-vend Session**
  ...
  For a multi-vend session, when the consumer is able to select multiple items in one transaction, enable the `multi_vend_support` flag. The session would then include a list of products.<br />You can review an example of it in the demo apps.
</Note>
...
<CardGroup cols={2}>
  <Card title="Get started" icon="rocket" href="/docs/integrate-pos-device/marshall/get-started/marshall-get-started">
    Get up and running with the Marshall SDK integration.
  </Card>

  <Card title="Methods & functions" icon="code" href="/docs/integrate-pos-device/marshall/methods-functions/marshall-methods-and-functions">
