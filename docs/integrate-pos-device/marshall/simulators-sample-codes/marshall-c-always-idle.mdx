...
metadata:
  og:description: "The SDK can also configure the peripheral to remain idle, meaning the transaction process would begin from the peripheral's end by sending the Vend."
---

The SDK can also configure the peripheral to remain idle, meaning the transaction process would begin from the peripheral's end by sending the "Vend Request" with the relevant product (instead of having the peripheral enable the device's reader upon a product selection, then having the device respond with "Begin Session" to the consumer's card presentation and only then have the peripheral send the "Vend Request").

This page will guide you on implementing an always idle state in C SDK in more detail. To better explain this process, the code implementation is divided into the following steps:
...
* `session_product`: Represents a single vending item.
...
This function determines the output stream for debug information, supporting different platforms:

* Linux/x86: Uses `printf()`.
* STM32 microcontroller: Uses `drv_usart_tx_data()`.
...
Since you want to leave the status in the always idle, set `config.always_idle` to `__true`.
...
Now, you can initialize the menu, calling the handlers created before. See the code below.
