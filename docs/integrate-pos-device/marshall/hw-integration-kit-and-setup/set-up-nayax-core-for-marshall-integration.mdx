...
keywords: ["marshall", "marshal", "MDB RX Timeout", "vend result timeout", "session timeout", "keep alive", "VPOST button function", "enable alerts"]
metadata:
  og:description: "Once you have completed the Marshall installation process, follow the steps below to configure the Nayax device to support the Marshall functionality."
---
...
3. Create a machine. If you need help, see the [**Create New Machine**](/docs/manage-data-operations/lynx-api/machines/create-new-machine-guides) guide.
...
Once a session has timed out, the device would settle the transaction on its own and send a "Status" command to the peripheral in order to notify it about the settlement taking place. This would let the peripheral know that it no longer needs to settle said transaction (as it has already been settled), and should he try to settle the already-settled transaction he would get "settlement error" (as the device couldn't find said session to settle it).
</Note>

Expect the session to expire slightly later than the value you set. The device checks its open sessions once a minute, so settlement can lag the configured timeout by up to a minute:

* A 1 minute timeout expires between 1 minute and 1 minute 59 seconds after the session starts.
* A 5 minute timeout expires between 5 minutes and 5 minutes 59 seconds.
* A 1 hour timeout expires between 1 hour and 1 hour 59 seconds.

## Configure Vend Result Timeout

The **Vend Result Timeout** sets how long the device waits for a vend response from the peripheral ("Vend Approved" or "Vend Failure") after it sends "Vend Approved" to the peripheral.

The maximum value is 65535 seconds, which is 18 hours, 12 minutes, and 15 seconds. If your machine needs a longer window than that, use [Multi-Session](/docs/integrate-pos-device/marshall/payment-flows/marshall-multi-session) instead, which runs on the **Session Time Out** parameter above and reaches considerably longer limits.

## Configure the Acquirer Approval Timeout

While the device waits for the acquirer, it shows "Authorizing, Please wait". The message stays up until a response arrives or the timeout is reached.

When the device can reach the Nayax servers normally, the round trip takes a few seconds, usually no more than five. Communication problems between the device and the servers over GSM, VPN, MQTT, or Ethernet stretch that out, either on the way to the servers or on the way back.

The timeout itself is the **MDB RX Timeout** parameter, and its default value is 60 seconds.

<Frame><img src="/images/docs/faq-general/image6.png" alt="MDB RX Timeout parameter in Nayax Core" /></Frame>
...
Read more on that in the [Stability/ starvation issues](/docs/integrate-pos-device/marshall/additional-resources/stablity-issues#/) section.
</Note>
...
## Remove the Cancel Button During Authorization

The device can show an **X** button while a transaction is being authorized, which lets the consumer cancel at that point.

To remove it, set the **VPOST button function** parameter to **VPOS cancel button during authorization**.
...
Alerts sent from the SDK only reach Nayax Core when the machine carries the configuration below. Apply it even if your machine is not an EV charger.

<Frame><img src="/images/docs/faq-general/image2.png" alt="Alert configuration on the machine in Nayax Core" /></Frame>

For the alert IDs your code can send and how to send them, see [General Marshall Events](/docs/integrate-pos-device/marshall/methods-functions/general-marshall-events).
