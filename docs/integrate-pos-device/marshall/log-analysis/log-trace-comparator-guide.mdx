...
- **Trace strip**: one dot per checkpoint, <Badge color="green">Green</Badge> pass, <Badge color="orange">Amber</Badge> warning, <Badge color="red">Red</Badge> fail, <Badge color="gray" variant="outline">Hollow gray</Badge> not applicable, in the same order the checkpoints are listed below.
...
The tool is a fast first-pass filter. It doesn't replace professional judgment. Before reporting an issue to a customer, check this list:
...
- [Log Trace Comparator Tool](/docs/integrate-pos-device/marshall/additional-resources/log-trace-comparator): the tool itself, with a short intro and results legend.
