...
As can be seen in the diagram above, once the Display Control command has been sent, the device would respond with a Display Status command, letting it know whether or not the device received said command and its validity.

### Starting a Transaction

Once a consumer has selected a product (screen) on the device, the device would respond with information to the peripheral regarding which screen was selected, to which it should respond with a "Vend Request" with the relevant product code and price in order for the transaction process to begin.
