...
metadata:
  og:description: "The Marshall implementation of ExternalSettlement separates the authorization and settlement phases of a transaction. While the Marshall peripheral."
---
...
- Should you like the settlement price to not necessarily be equal to the authorized amount (settlement price ≤ authorized price)- you should enable the "price not final" flag in the Marshall SDK.
...
//the fields are (from left to right): product code (2 bytes long, meaning value up to 65535), quantity (2 bytes), unit of measure (1 byte), price (2 bytes, meaning value up to 65535)
```
...
21. The 3rd Party Server sends an [ExternalSettlement](/reference/spark/transactions/externalsettlement) request. It must contain: `NayaxTransactionId`, `SiteId` (information retrieved from the transaction's Transfer Data's command) and be sent within 48 hours of the authorization of the consumer's card.
    - Note: should you like the settlement price to not necessarily be equal to the authorized amount (settlement price ≤ authorized price)- you should enable the "price not final" flag in the Marshall SDK.
