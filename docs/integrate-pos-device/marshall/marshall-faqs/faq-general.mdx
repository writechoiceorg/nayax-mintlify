---
title: "General"
keywords: ["marshall", "marshal", "Marshall FAQ", "product map", "gtrace", "last alerts", "static ip", "cable SKU", "firmware version update", "MDB RX Timeout", "RFID cards", "magstripe cards", "vending machine controller", "VMC"]
metadata:
...
Nayax' reader is working in frequency **13.56 MHz and supports ISO 14443 Type A and Type B cards (e.g. MiFare Classic and MiFare Plus) + ISO 15693 (e.g. HID iClass)**. The most popular and simple RFID card supported by Nayax' card reader is of type: MiFare Classic 1K (or 4K).
...
Setting up a product in the Product Map:\
...
**C SDK:**
Our SDK developer estimates its memory usage at around 1KB of RAM and every 5ms, timer tick of around 100 lines of code (actual machine code depends on compiler and platform).
...
In such case they should use the Java SDK as is with the small change for changing the low-level serial port (as you’re using an Android platform) with the code in the demo called "nayax-android-marshall-demo\_0\_1\_6\_02"
