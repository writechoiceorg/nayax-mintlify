...
After the "Vend Request" is sent and a consumer presents his card the Transfer Data command would be sent and the peripheral would get the card's details and would forward them (on its own, unrelated to the SDK) to the desired 3rd party server.
...
public void onSessionBegin(int funds_avail)
{
    ...
    else
    {
        ...
        // example: approve mifare/mag card externally (vmc authenticates)
...
If you set the timeout to be 5 minutes, the session would expire anywhere between 5 minutes to 5 minutes and 59 seconds from starting it.\
