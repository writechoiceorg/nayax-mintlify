...
metadata:
  og:description: "The terminal uses the TransactionCallback endpoint to notify the integrator about the result of a card tap. It is triggered automatically when a user."
---
...
The Spark platform sends a POST request to your configured `TransactionCallback` endpoint. 

The call would be sent to the following endpoint:

`(Customer's URL)/TransactionCallback`
