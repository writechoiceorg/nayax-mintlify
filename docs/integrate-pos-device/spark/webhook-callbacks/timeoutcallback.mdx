...
metadata:
  og:description: "Pre-Authorization Only This method is only relevant for transactions with Pre-Authorization. The TimeoutCallback API call notifies that the payment."
---
...
Spark sends a POST request to your configured `TimeoutCallback` endpoint. 

The call would be sent to the following endpoint:

`(Customer's URL)/TimeoutCallback`

The request body is as in the example below:
