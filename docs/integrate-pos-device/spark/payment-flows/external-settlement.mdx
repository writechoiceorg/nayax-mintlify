...
<Danger>
  **Note**
  ...
  In addition, in case of using External Settlement with Marshall Multisession, note that you should not send a "Close Session" command for settlement.
</Danger>
...
<CardGroup cols={3}>
  <Card title="StartAuthentication" icon="key" href="/reference/spark/transactions/startauthentication">
    Authenticates the 3rd-party server before an external settlement or cancel request.
  </Card>

  <Card title="ExternalSettlement" icon="money-check-dollar" href="/reference/spark/transactions/externalsettlement">
    Completes a transaction that was authorized outside a standard Spark flow.
  </Card>

  <Card title="ExternalCancel" icon="ban" href="/reference/spark/transactions/externalcancel">
