...
metadata:
  og:description: "SMC (Self-Managed Cards) is a Spark feature that enables integrators to utilize closed-loop cards, such as RFID tags, company-issued badges."
---

SMC (Self-Managed Cards) is a Spark feature that enables integrators to use closed-loop cards, such as RFID tags, company-issued badges, or loyalty cards, within Spark's Remote/Device start flows and Device Stop flow. Rather than relying on Nayax or the acquiring bank to authorize the card, your system is responsible for validating and authorizing it. It is ideal when:
...
To support this feature, you must use a dedicated “Payment Method”. Machines supporting this Payment Method will be able to support Spark Self-Managed cards and Credit Cards only (no additional Prepaid cards of different Providers).
