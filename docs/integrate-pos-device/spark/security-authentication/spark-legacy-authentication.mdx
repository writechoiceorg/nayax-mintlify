...
metadata:
  og:description: "Spark uses a two-part security mechanism: Token-based identification to associate the request with your account. HMAC signature using a shared secret."
---
...
<Frame>
  <iframe
    src="/tools/spark-auth-tool.html?auth=legacy"
    title="Spark Auth Tool (Legacy)"
    loading="lazy"
    style={{ width: "100%", height: "820px", border: 0 }}
  />
</Frame>

Need more room? <a href="/tools/spark-auth-tool.html?auth=legacy" target="_blank" rel="noopener">Open the tool in full screen</a>.
