...
6. You can also continue to run another authorization. The previous transaction may be kept open (Authorized).
...
### External settlement example settings

To simulate [External Settlement](/docs/integrate-pos-device/emv-core/payment-flows/pre-authorization-with-external-settlement) from the demo application, get the following parameters from Nayax support and set them in the sample app's `settings.java` file:
