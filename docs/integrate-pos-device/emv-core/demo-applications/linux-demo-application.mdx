...
The zip contains a project with makefiles, a library, and a sample application with C source code:

* The library `libotikiosk` converts the Socket JSON API to C functions.
* `otiKioskDemo` supports two operating modes: Single API flow and Auto Mode.
...
2. After selecting the command, it runs the command and returns to step 1.
...
Follow these steps to compile the demo from source:
