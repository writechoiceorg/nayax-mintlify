...
* **Socket usage:**
  ...
  * 2 short-lived sockets (few seconds, one or more request/response cycles) — may work in parallel.
...
<Note>
  It is highly recommended to use the sample host implementation provided with the SDK rather than develop it from scratch.
</Note>

## SDK Delivery

The SDK package includes the following folders and files:
...
The Kiosk ID is taken from the Reader Serial Number (SN) and can be obtained by:
