...
The configuration file contains parameters and initial settings that are managed from the Nayax backend server (Nayax Core). Settings may vary between customers and machines.
...
| `PAYMENT_SVC_SYNCHRONOUS` | Don't start a new transaction before the previous one is closed on the server. Note: for asynchronous after unexpected reset, also set `PMT_KIOSK_AUTOCONFIRM=0`. | Bool (1/0) | 0 | — |
