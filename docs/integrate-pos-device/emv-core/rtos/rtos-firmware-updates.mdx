...
<Steps>
  ...
  <Step title="Select the POS requested FW version">
    ...
    * **For UNO-mini:**
      * RTOS SDK firmware starts with `EMVSC_` (e.g., `EMVSC_v071652`)
    ...
    * If more than one firmware is available for your device, take the latest one.
    ...
    <Warning>
      RTOS SDK v1.0 should stay on firmware `EMVSC_v071652.oef`. Don't upgrade it to `EMVSC_v071749`, the firmware released for SDK v1.5.
    </Warning>
...
| :------ | :--------------- | :------------------------ | :------ |
| USA | Heartland Nayax EMV | `HTL_` | HTL\_29 |
| Canada | CityCardPayment (Nuvei) | `NUV_` | NUV\_16 |
| Europe, UK | Credorax (Finaro/Shift4) | `FIN_` | FIN\_18 |
| Other countries | Stubb billing (test only) | `FIN_` | FIN\_18 |
