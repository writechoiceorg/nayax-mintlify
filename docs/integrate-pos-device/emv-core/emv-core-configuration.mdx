...
|                       | `PAYMENT_SVC_SYNCHRONOUS` | Waits for the current transaction to close on the server before starting a new one. | Bool (1/0) | 0                             | For asynchronous transactions in all cases, including after an unexpected device reset, also set `PMT_KIOSK_AUTOCONFIRM=0`. |
...
|                       | `CLP_MIFARE_READ`       | Format: `K[A`\|`B][key_num]B[block_num]...` (for example, `KA01B10B14,KB05B20B30` reads blocks 10 and 14 with key A number 1, and blocks 20 and 30 with key B number 5). | String     | —                             | Read blocks are returned in `TransactionComplete`'s `mifare_read` field. |
