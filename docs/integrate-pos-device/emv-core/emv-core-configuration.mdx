...
|                       | `PAYMENT_SVC_SYNCHRONOUS` | Waits for the current transaction to close on the server before starting a new one. | Bool (1/0) | 0                             | `PMT_KIOSK_AUTOCONFIRM` has no effect on this platform since SDK v1.5. |
