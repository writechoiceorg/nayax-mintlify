...
<Note>
  The manual doesn't specify how long the data set here remains in effect (next transaction only, current session, until overwritten, etc.). Confirm scope with Nayax before publishing guidance on when to call this versus the inline `additionalReceiptData` parameter.
</Note>
