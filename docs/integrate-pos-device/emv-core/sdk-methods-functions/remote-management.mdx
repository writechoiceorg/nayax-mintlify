...
keywords: ["kibana", "remote management", "TMS", "terminal management system", "diagnostics", "monitoring", "GetKioskID", "GetStatus", "GetVersion", "firmware version"]
metadata:
  og:description: "The EMV Core SDK includes management features that streamline device configuration, firmware updates, and diagnostics. These capabilities reduce."
---
...
* [SendToReader](#sendtoreader)
* [SetAdditionalReceiptData](#setadditionalreceiptdata)
...
<Tabs>
  <Tab title="JSON">
    ...
    ```json
    {"jsonrpc":"2.0","method":"GetAttribute", "params": {"Key":"PRE_AUTH_AMUONT"}, "id":12}
    ```
    ...
    **Parameters:**
    ...
    | :-------- | :------------------------------------------------------------------------ |
    | **id**    | Command ID.                                                               |
    | **key**   | Label of the requested configuration attribute (e.g., `PRE_AUTH_AMUONT`). |
  </Tab>
  ...
  <Tab title="Android">
    ...
    **Parameters:**
    ...
    | Parameter | Description                                                                                                                                                         |
    | :-------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
    | `key`     | The attribute key in the configuration file. For example, if the config file includes `PRE_AUTH_AMUONT=100`, providing the key `PRE_AUTH_AMUONT` will return `100`. |
  </Tab>
  ...
  <Tab title="Linux">
    ...
    **Parameters:**
    ...
    | Parameter        | Description                                                                                                                             |
    | :--------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
    | `attribute_name` | A null-terminated string representing the attribute key in the configuration file (e.g., `"PRE_AUTH_AMUONT"`).                          |
...
<Tabs>
  ...
  <Tab title="Android">
    ```java Java
    PollStatus GetPollStatus() throws EmvCoreException;
    ```
  </Tab>
...
`PollStatus` has two fields: `polling` (Boolean, `true` if EMV Core is currently polling for a card) and `resources` (String, resource availability: `Ready`, `NoReader`, or `NoNetwork`).
...
Sends a direct command to the payment card reader hardware. This provides low-level access for custom commands, configuration, and diagnostics.

See the `SendToReader` API signature in the code below:

```java Android
SendToReaderResponse SendToReader(String scope, String tag, String data);
```
...
| Parameter | Description |
| --------- | ----------- |
| `scope` | Command scope. `GET` reads data (all tags allowed). `DO` executes a command (only whitelisted tags allowed). |
| `tag` | Reader tag identifier in hex (for example, `DF4D` for firmware version). |
| `data` | Command payload as a hex string. Use an empty string for `GET` operations. |

**Returns:** A `SendToReaderResponse` with:

| Field | Description |
| --------- | ----------- |
| `status` | Numerical status code: `0` (SUCCESS), `1` (FAILED), `2` (TIMEOUT), `3` (NO_ACK), `4` (NACK), `5` (UNSUPPORTED), `6` (UNAUTHORIZED), or `7` (ERROR). |
| `data` | Hex-encoded response data on success, or an error message otherwise. |
...
Sets additional receipt data to be included in transactions. This is a separate command from the `additionalReceiptData` parameter available inline in [`PreAuthorize` and `ConfirmTransaction`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1).

See the `SetAdditionalReceiptData` API signature in the code below:

```java Android
JSONObject SetAdditionalReceiptData(List<AdditionalReceiptDataPrms> additionalReceiptData) throws EmvCoreException;
```
...
| Parameter | Description |
| --------- | ----------- |
| `additionalReceiptData` | List of `AdditionalReceiptDataPrms` objects, each with a key (String) and value (Object) pair to include in the receipt. |

**Returns:** A `JSONObject` with the configured receipt data.
