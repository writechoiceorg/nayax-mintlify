...
The `IncrementalAuthorizationResponse` event is triggered when an [`IncrementalAuthorization`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1#incrementalauthorization) call completes, whether it succeeds or fails.

<Note>
  This callback is reserved for a future multi-step incremental authorization flow. For the single-step flow Nayax Core handles automatically today, see [Single-step incremental authorization](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1#single-step-incremental-authorization).
</Note>
