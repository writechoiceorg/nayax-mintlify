...
<Tabs>
  ...
  <Tab title="Android">
    ...
    Where `tokenDetails` has two fields:

    | Parameter  | Description                                                                                                 |
    | :--------- | :------------------------------------------------------------------------------------------------------------ |
    | `CardToken` | Base64 string for payment cards, or the card UID for closed loop (Mifare) cards.                              |
    | `CardType`  | Card scheme (for example, EMV, MasterCard, Visa). For closed loop cards, the card technology (for example, Type A, Mifare). |
  </Tab>
...
<Warning>
  This event is only available for the Android operating system.
</Warning>

The `IncrementalAuthorizationResponse` event triggers when an [`IncrementalAuthorization`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1#incrementalauthorization) call completes, whether it succeeds or fails. It's reserved for a future multi-step flow and isn't required today: for the current single-step behavior, see [Single-step incremental authorization](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1#single-step-incremental-authorization).

<Note>
  Nayax doesn't yet support the multi-step flow server-side. `IncrementalAuthorizationResponse` won't fire beyond what's described above.
</Note>
...
| --------- | ----------- |
| `Status` | `OK` or `Error`. |
| `Amount` | The incremental amount, in major units (for example, 1.50 for $1.50). |
| `TransactionReference` | Transaction reference from the original `IncrementalAuthorization` call. |
...
`EmvCoreStatus` is an enumeration representing the current state of the system. See the possible result values in the table below:
...
| `NoReader`           | No card reader is currently connected.         |
| `NoTerminalId`       | Indicates the EMV Core does not have a configured Terminal ID. |
| `NoNetwork`          | No network connection is available.            |
| `NoTransaction`      | No transaction is currently in progress or was found. |
...
<Tabs>
  <Tab title="JSON">
    ...
    **Parameters:**
    ...
    | :----------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------- |
    | **id**                   | Command ID.                                                                                                                                    |
    | **status**               | Transaction result: `OK`, `Declined`, `Error`, `Timeout`, `Cancelled`, `LocalCLP`, or `PollContinuous`.                                                         |
    | **errorCode**            | Numerical error code. `0` when there is no error.                                                                                             |
...
If EMV Core doesn’t get an answer, it will re-send 2 times (for a total of 3 messages), and if it still doesn’t get an answer, it will assume the message received and will not cancel the transaction.
    ...
    | `status`                  | Possible statuses: OK, Declined, Error, Timeout, Cancelled, LocalCLP, PollContinuous.                                                                                        |
    ...
    | `TransactionID`           | Transaction ID. Use this value in `ConfirmTransaction` or `VoidTransaction`.                                                                   |
    | `Transaction_Reference`   | Transaction reference. Use this value in `ConfirmTransaction` or `VoidTransaction`.                                                                                                 |
    ...
    | `mifare_read`             | Optional. JSON with the results of Mifare blocks read, according to the `CLP_MIFARE_READ` configuration attribute. Empty string when not configured. Returns a JSON object keyed by UID and by block number (for example, `B10`, `B14`).                                                                                                                                    |
