...
<Tabs>
  <Tab title="JSON">
    ...
    **Parameters:**
    ...
    | **isFinalAmount**         | `true`: fixed amount, indicates ‘pre-selection’ flow. `false`: amount may change, indicates ‘pre-authorization’ flow. When used with `deferred=false`, the host must call `ConfirmTransaction` or `VoidTransaction` in both cases. When used with `deferred=true`, no need to call `ConfirmTransaction` or `VoidTransaction` — transaction will be automatically settled. |
