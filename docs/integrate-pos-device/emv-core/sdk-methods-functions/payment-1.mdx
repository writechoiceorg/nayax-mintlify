...
The `IncrementalAuthorization` method adds an incremental charge to an existing authorized transaction, and delivers the result through the [`IncrementalAuthorizationResponse`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/event-callbacks#incrementalauthorizationresponse) callback. It's reserved for a future multi-step flow and isn't required today: for the current single-step behavior, see below.
...
<Note>
  Nayax doesn't yet support the multi-step flow server-side. Calling `IncrementalAuthorization` today has no effect beyond what's described above.
</Note>
