...
Add an incremental charge to an existing authorized transaction. Use this when the final amount is higher than the initially authorized amount. The result is delivered via the `IncrementalAuthorizationResponse` callback.
