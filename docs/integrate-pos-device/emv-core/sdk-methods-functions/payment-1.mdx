---
title: "Payment"
keywords: ["getcardtoken", "GetCardToken", "card token", "CancelTransaction", "ConfirmTransaction", "PreAuthorize", "VoidTransaction", "card tokenization", "pre-authorization", "CardTokenReceived"]
metadata:
...
The EMV Core SDK provides APIs for managing payment transactions. These APIs let you integrate cashless payment functionality into applications, supporting both open- and closed-loop systems. The SDK handles secure, efficient, and flexible payment processing while offloading complexity to the Nayax backend.
...
<Tabs>
  <Tab title="JSON">
    ...
    **Parameters:**
    ...
    | **productID**             | Optional parameter: ID of the product selected by the consumer. This number will be saved on the server and available for reports. Before SDK v1.5, `productID` also controlled whether the pre-authorized amount was final; that behavior is now controlled by `isFinalAmount` in `PreAuthorize`. |
...
Add an incremental charge to an existing authorized transaction. Use this when the final amount is higher than the initially authorized amount. The result is delivered via the [`IncrementalAuthorizationResponse`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/event-callbacks#incrementalauthorizationresponse) callback.

### Single-step incremental authorization

For most use cases, you don't need to call this API directly. Nayax Core handles a single-step incremental authorization automatically whenever you settle a transaction for more than the pre-authorized amount:

1. Call `PreAuthorize` (for example, for $10.00) and read the `IncrementalMaxDefaultCredit` value from the resulting `TransactionComplete` event (for example, $50.00).
2. Call `ConfirmTransaction`, or send an [External Settlement](/docs/integrate-pos-device/emv-core/payment-flows/pre-authorization-with-external-settlement) request from your server, with an amount up to `IncrementalMaxDefaultCredit` (for example, $15.03).
3. Nayax Core automatically sends an incremental authorization request to the processor for the difference (in this example, $5.03).
4. If the incremental authorization succeeds, Nayax Core settles the transaction for the final amount ($15.03).
5. If it fails, Nayax Core settles the transaction for the original pre-authorized amount ($10.00).

<Note>
  The `IncrementalAuthorization` method and `IncrementalAuthorizationResponse` callback below are reserved for a future multi-step incremental authorization flow. They aren't required for the single-step flow described above.
</Note>
...
See the `IncrementalAuthorization` API signature in the code below:
...
| Parameter | Description |
| --------- | ----------- |
| `amountMinorUnits` | Additional amount to charge, in minor units (cents). |
| `productID` | Optional. Product ID for reporting. |
| `transactionReference` | Transaction reference from `TransactionComplete` (`TransactionID` field). |
...
<Tabs>
  ...
  <Tab title="Android">
    ...
    **Parameters:**
    ...
    | `additionalReceiptData` | Optional. JSON object with extra receipt data passed to the PSP and included in the eReceipt. Pass null if not used. |
    | `isDeferred` | Optional. `true`: Deferred Authorization (Transit mode). Card read offline, authorization queued for settlement. See [Deferred Transaction](/docs/integrate-pos-device/emv-core/payment-flows/deferred-transaction) for details. |
    | `isFinalAmount` | `true`: fixed amount, indicates `pre-selection` flow. `false`: amount may change, indicates `pre-authorization` flow. When used with `isDeferred=false`, the host must call `ConfirmTransaction` or `VoidTransaction` in both cases. When used with `isDeferred=true`, no need to call `ConfirmTransaction` or `VoidTransaction`. The transaction settles automatically. In SDK v1.5, `isDeferred=true` is only supported with `isFinalAmount=true`. |
  </Tab>
  ...
  <Tab title="Linux">
    ...
    **Parameters:**
    ...
    | `is_final_amount`            | `true`: fixed amount, indicates the pre-selection flow. `false`: amount may change, indicates the pre-authorization flow. In SDK v1.5, `is_deferred=true` is only supported with `is_final_amount=true`. |
  </Tab>
</Tabs>

<Warning>
  **Before SDK v1.5:** setting `productID` to a non-zero value in `PreAuthorize` marked the pre-authorized amount as final, and it couldn't be changed in `ConfirmTransaction`. To indicate a non-final amount, you set `productID = 0` in `PreAuthorize` and provided the actual product ID in `ConfirmTransaction`.

  **As of SDK v1.5**, whether the amount is final is controlled by `isFinalAmount` in `PreAuthorize` instead (see the parameter table above). `productID` is now used only for reporting and can be set independently in either call.
</Warning>
