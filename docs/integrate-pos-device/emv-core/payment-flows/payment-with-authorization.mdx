...
<Note>
  If you prefer, you can perform the authorization and settlement in just one step. To learn more about this process, visit the [Single Step Payment](/docs/single-step-payment) page.
</Note>
...
1. The host machine calls the [`PreAuthorize`](/docs/payment-1#preauthorize) method using the EMV Core API. This is the only action performed by the host machine.
...
4. The Host application can now confirm the transaction. The [`ConfirmTransaction`](/docs/payment-1#confirmtransaction) operation must be used to inform the transaction ID and the amount to be charged.
