...
In the first step, the host machine uses the EMV Core to authorize a specified amount for the payment operation with the buyer's card. See the following diagram:
...
8. Nayax EMV Core will use the event or callback to inform the host application that the transaction was completed, including the transaction ID and transaction data.
...
After receiving the transaction approval, the vendor can proceed with their selling flow. They can provide the product, start charging the vehicle, or perform any other operation. The vendor can also keep the transaction id and transaction data on their servers.

### Step 2: Settlement

The settlement step is when the final amount is defined and charged to the buyer's card. The settlement starts after the product is sold or the operation, such as EV charging, is completed. At this moment, the host machine can confirm the transactions. See the diagram below:
...
4. The Host application can now confirm the transaction. The [`ConfirmTransaction`](/docs/integrate-pos-device/emv-core/sdk-methods-functions/payment-1#confirmtransaction) operation must be used to inform the transaction ID and the amount to be charged.

<Note>
  The final amount can differ from the amount informed in the pre-authorization. To avoid problems, Nayax suggests using the maximum value at the authorization phase if the final amount related to your selling process may vary.
</Note>
...
7. Nayax servers will immediately return an acknowledgment confirmation to EMV Core. This is necessary to ensure that EMV Core does not need to resend the confirmation message.

Nayax will handle the settlement transaction to ensure the funds are transferred correctly.

<Note>
  If you need to cancel a previously pre-authorized transaction, instead of the host application sending a confirm transaction operation to EMV Core, it needs to send a void operation.
</Note>
