...
<Warning>
  The host machine does not know if the client will use an open loop (credit or debit cards) or a closed loop (MIFARE cards) transaction. This information is only available after the Nayax EMV Reader actually reads the client card. Therefore, the host machine must be prepared to process both operations after the Nayax EMV Core identifies the operation type.
</Warning>
