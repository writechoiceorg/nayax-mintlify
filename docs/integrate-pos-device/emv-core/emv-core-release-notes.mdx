---
title: "Android & Linux SDK Release Notes"
metadata:
  og:description: "This section provides a comprehensive history of updates, enhancements, and stability improvements to EMV Core across our supported platforms."
---
...
<Accordion title="Improvements" icon="rocket">
  ...
  ***
  ...
  You can now manage closed-loop and pre-paid cards through Nayax Core by setting `CLP_MIFARE_ENABLED=1`. This allows the Nayax server to process CLP cards directly.
...
<Accordion title="Fixes" icon="wrench">
  ...
  Fixed a UI synchronization issue where a `PreAuthorize` command initiated without a network connection would incorrectly display "DECLINED" before switching to "SUCCESS" once the network recovered. We also fixed a critical crash that occurred when `PreAuthorize` was called multiple times while offline.

  ### **Reader Boot and Core Invocations (#7199, #7272)**

  Fixed a bug where the EMV Core reported `Ready` before the reader finished booting. Additionally, we implemented protection against multiple simultaneous invocations of the EMV Core to prevent resource conflicts.
