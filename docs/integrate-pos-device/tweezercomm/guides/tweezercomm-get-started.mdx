...
* `method`: Specifies the method to be used. See [Global Payment Methods](/docs/transaction-methods), [Israel Payment Methods](/docs/tweezercomm-israel-payment-methods), and [Device Methods](/docs/device-methods) for a list and description of available methods.
...
* `params`:  This is an array divided into two parts.
  * The first specifies the payment service to be used:
    * `engine` for global transactions. See [Global Payment Flows](/docs/payment-flows-copy-1) for more details.
    * `ashrait` for Israel transactions. See [Israel Payment Flows](/docs/tweezercomm-israel-payment-flows) for more details.
