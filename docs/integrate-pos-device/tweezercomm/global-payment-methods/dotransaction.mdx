...
<Tabs>
  ...
  <Tab title="Pre-Authorization">
    For a initiating a Pre-Authorization transaction, set the `tranType` parameter to `3`. The code block shows a request example for initiating a Pre-Authorization transaction:
