...
<Tabs>
  ...
  <Tab title="Pre-Authorization">
    For initiating a Pre-Authorization transaction, set the `tanType` parameter to `3`. The code block shows a request example for initiating a Pre-Authorization transaction:
