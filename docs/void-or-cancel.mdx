...
The Refund request uses the same endpoint as the initial  [ `/charge-token` ](https://developerhub.nayax.com/reference/post_charge-token)  request. The API determines it's a settlement based on the `RequestType` value within the `BasicInfo` object.
