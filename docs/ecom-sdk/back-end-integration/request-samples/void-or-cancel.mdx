...
metadata:
  og:description: "The Void and Cancel requests both reverse a transaction, but their use depends critically on the original transaction's status. Both operations use."
---
...
The Void or Cancel request uses the same endpoint as the initial [`/charge-token`](/reference/ecom/payments/charge-token) request. The API determines it's a void or cancel based on the `RequestType` value within the `BasicInfo` object.
...
Note that the `RequestType` is set to the value corresponding to a Cancel (4) or Void (5). The `Amount` specifies the full amount of the original authorization/sale.
...
| :------------------- | :------- | :---------------------------------------------------------------- |
| `RequestType`        | Int32    | 4 for `Cancel` or 5 for `Void`.                                   |
