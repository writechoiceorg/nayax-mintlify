...
The Refund request uses the same endpoint as the initial [ `/close-transaction`](/reference/ecom/payments/close-transaction) request. The API determines it's a refund based on the `RequestType` value within the `BasicInfo` object.
