...
The Refund request uses the same endpoint as the initial [ `/charge-token`](/reference/ecom/payments/charge-token) request. The API determines it's a refund based on the `RequestType` value within the `BasicInfo` object.
