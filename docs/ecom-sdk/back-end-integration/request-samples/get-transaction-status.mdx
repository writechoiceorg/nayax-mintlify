...
metadata:
  og:description: "This read-only query checks the current status of a previously initiated transaction session. It allows the merchant system to proactively poll the."
---
...
This request targets a dedicated [status endpoint](/reference/ecom/management/retrieve-transaction-status-by-session-id). It uses the `NayaxSessionId `as the primary identifier for the transaction being queried.
