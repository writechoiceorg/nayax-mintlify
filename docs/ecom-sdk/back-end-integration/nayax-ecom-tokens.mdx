...
title: "Nayax Tokens"
metadata:
  og:description: "The Nayax eCom SDK uses tokenisation for Credential on File (COF) transactions. When a COF initialisation is triggered, the eCom API responds with a."
---

The Nayax eCom SDK uses tokenization for Credential on File (COF) transactions. When you trigger a COF initialization, the eCom API responds with a Nayax card token to the eCom SDK. This token is stored within Nayax's systems, and your application can use it for recurring transactions, as it signifies the cardholder's authorization for you to store and debit their card in the future.
...
Use the [`/charge-token`](/reference/ecom/payments/charge-token) endpoint for recurring payments. Include a card token in the request. These calls run only on the server.
...
The delete-a-card-token endpoint lets you delete a saved token. The request must include the following in the `BasicInfo` and `MachineInfo` objects:
...
<Columns cols={2}>
  ...
  <Card icon="link" title="Webhook Notifications" href="/docs/ecom-sdk/back-end-integration/webhook-notifications" />
</Columns>