...
metadata:
  og:description: "The SDK provides a callback interface that delivers the transaction's final status. PaymentResultCallback for Android. PaymentResult for iOS."
---
...
You receive the `PaymentCompleteData` data structure when a payment is successful. It contains the following information:
...
You receive the `PaymentFailData` data structure when a payment fails. It contains the following information:
...
The SDK provides several error types to help diagnose issues during a transaction:
