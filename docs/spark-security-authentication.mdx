...
This signature mechanism is required for all Spark endpoints. Nayax rejects requests without a valid signature with `401 Unauthorized`.
</Warning>
...
2. Extract the 32 rightmost characters from your provided Token to create the 256-bit AES encryption key. For example, suppose the token is the following:
   ...
   ```
3. Encrypt the 64-character ciphertext using AES in ECB mode. Ensure the plaintext is padded (e.g., using PKCS7) to a multiple of 16 bytes. Base64-encode the resulting encrypted bytes for transmission. The output looks similar to the example below:
   ```
