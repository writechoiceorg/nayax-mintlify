...
The Settlement request uses the same endpoint as the initial [ `/charge-token`](/reference/post_charge-token) request. The API determines it's a settlement based on the `RequestType `value within the `BasicInfo `object.

Note the key differences from a [ `/charge-token`](/reference/post_charge-token) request:
