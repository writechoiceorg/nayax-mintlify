...
2. The Cortina API then initiates a service session using the [Start Session](/reference/post_cortina-startsession) endpoint and sends a [Credit Card Sale](/reference/post_cortina-creditcard-sale) request to the Integrator. This request includes all the necessary details to authorize the payment for the product or service.
...
6. Once the service is completed or the product has been dispensed, the Nayax Device sends a [Sale End](/reference/post_cortina-saleendnotification) request in the Cortina API. This indicates that the transaction is complete and that the customer has received the service or product.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [Credit Card Void](/reference/post_cortina-creditcard-void) endpoind to void the payment request.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived,   the Nayax Engine uses the [Credit Card Void](/reference/post_cortina-creditcard-void) endpoind to void the payment request. To indicate that the Void is sent due to timeout, the flag `isGatewayTimeout `will be sent as `True`.
</Warning>
...
2. The Nayax Engine performs a [Start Session Request](/reference/post_cortina-startsession) using Cortina API to establish a session for the transaction. The request is directed to the Integrator (the payment processor or gateway).
...
8. The Nayax Engine forwards the Settlement Request to the integrator using Cortina's API [Credit Card Settlement](/reference/post_cortina-creditcard-settlement) endpoint, which includes the final transaction amount to be settled.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [Credit Card Cancel](/reference/post_cortina-creditcard-cancel) endpoint to cancel the Pre Authorization and the transaction.
</Warning>

<Warning>
**Timeout Cancellation**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived, the Nayax Engine uses the [Credit Card Cancel](/reference/post_cortina-creditcard-cancel) endpoint to cancel the Pre Authorization and the transaction. To indicate that the cancellation is sent due to timeout, the flag `isGatewayTimeout `will be sent as `True`.
</Warning>
