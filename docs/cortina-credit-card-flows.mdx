...
2. The Cortina API then initiates a service session using the [Start Session](/reference/cortina/cortina/post-start-session) endpoint and sends a [Credit Card Sale](/reference/cortina/cortina-credit-card/cortina-credit-card-sale) request to the Integrator. This request includes all the necessary details to authorize the payment for the product or service.
...
6. Once the service is completed or the product has been dispensed, the Nayax Device sends a [Sale End](/reference/cortina/cortina/post-sale-end-notification) request in the Cortina API. This indicates that the transaction is complete and that the customer has received the service or product.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [Credit Card Void](/reference/cortina/cortina-credit-card/cortina-credit-card-void) endpoind to void the payment request.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived,   the Nayax Engine uses the [Credit Card Void](/reference/cortina/cortina-credit-card/cortina-credit-card-void) endpoind to void the payment request. To indicate that the Void is sent due to timeout, the flag `isGatewayTimeout `will be sent as `True`.
</Warning>
...
8. The Nayax Engine forwards the Settlement Request to the integrator using Cortina's API [Credit Card Settlement](/reference/cortina/cortina-credit-card/cortina-credit-card-settlement) endpoint, which includes the final transaction amount to be settled.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [Credit Card Cancel](/reference/cortina/cortina-credit-card/cortina-credit-card-cancel) endpoint to cancel the Pre Authorization and the transaction.
</Warning>

<Warning>
**Timeout Cancellation**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived, the Nayax Engine uses the [Credit Card Cancel](/reference/cortina/cortina-credit-card/cortina-credit-card-cancel) endpoint to cancel the Pre Authorization and the transaction. To indicate that the cancellation is sent due to timeout, the flag `isGatewayTimeout `will be sent as `True`.
</Warning>
