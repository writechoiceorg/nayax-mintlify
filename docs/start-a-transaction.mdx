...
The `/initialize` endpoint is also part of the process for tokenizing a card. If a Card-on-File (COF) initialization is triggered, the eCom API will respond with a Nayax card token to the Nayax SDK. See [Nayax Tokens](/docs/nayax-ecom-tokens) for more details.
</Note>
...
<Columns cols={2}>
  <Card icon="link" title="Nayax Tokens" arrow href="/docs/nayax-ecom-tokens" />

  <Card icon="link" title="Webhook Notifications" arrow href="/docs/webhook-notifications" />
</Columns>