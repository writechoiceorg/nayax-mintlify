...
1. Prepare the JSON body with all the necessary fields (e.g., `actorId`, `Cipher`, etc.). Here’s a simplified example for the [`/validateMerchant`](https://developerhub.nayax.com/reference/post_validate-merchant) endpoint:
...
For this example, the [`/validateMerchant`](https://developerhub.nayax.com/docs/validateMerchant) method was used, but the process is relevant for any eCom SDK method request and response.
...
<Columns cols={2}>
  ...
  <Card title="Tokenization" href="/v1.0/docs/tokenization" />
</Columns>