...
5. At a later time, the POS sends another [`doTransaction()`](/docs/dotransaction-ashrait) request, this time with a parameter (`Trancode=3`) to initiate the completion of the pre-authorized transaction.
...
8. At the end of the day, the POS sends a [`doPeriodic()`](/docs/doperiodic) request. The Nayax Device communicates with the payment server to perform the settlement of the day's transactions and sends a[`doPeriodic()`](/docs/doperiodic) result back to the POS.
