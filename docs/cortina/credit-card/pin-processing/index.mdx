...
metadata:
  og:description: "A PIN (Personal Identification Number) is a numerical code used to authorize and verify the cardholder in transactions done with debit and credit."
---
...
When a Cardholder enters their PIN at a terminal, the Nayax terminal encrypts the PIN using a DUKPT algorithm and sends it to the Nayax Engine as a PIN block, which is the encrypted PIN string.
