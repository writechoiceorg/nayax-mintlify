...
title: "Void"
metadata:
  og:description: "Cortina DQR uses the Void command to notify the integrator in order to initiate the cancelation process with the Cortina payment provider whenever on."
---

Cortina DQR uses the `Void` command to notify the integrator in order to initiate the cancelation process with the Cortina payment provider whenever one of the below scenarios takes place:
...
This command is essential for having the payment provider aware that the transaction was cancelled on Nayax's end (as well as from the consumer's perspective), and is done in order for the payment provider to complete the cancellation on its end.
