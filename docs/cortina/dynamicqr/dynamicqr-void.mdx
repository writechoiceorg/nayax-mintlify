...
title: "Void"
metadata:
  og:description: "Cortina DQR uses the Void command to notify the integrator in order to initiate the cancellation process with the Cortina payment provider whenever on."
---

Cortina DQR uses the `Void` command to notify the integrator in order to initiate the cancellation process with the Cortina payment provider whenever one of the below scenarios takes place:
