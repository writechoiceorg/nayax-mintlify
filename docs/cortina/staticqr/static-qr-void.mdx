...
title: "Void"
metadata:
  og:description: "Cortina SQR uses the Void command to notify the integrator in order to initiate the cancellation process with the Cortina payment provider whenever on."
---

Cortina SQR uses the `Void` command to notify the integrator in order to initiate the cancellation process with the Cortina payment provider whenever one of the below scenarios takes place:
