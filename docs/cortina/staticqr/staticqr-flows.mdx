...
title: "StaticQR"
metadata:
  og:description: "Static QR codes provide a straightforward, secure way to process payments via wallet applications, enabling customers to scan a code displayed on a machine."
---
...
In case the product fails to be dispensed to the client, the Nayax Engine uses the [StaticQR Cancel](/reference/cortina/staticqr/cortina-staticqr-cancel) endpoint to void the payment request.
</Warning>
...
In case the product fails to be dispensed to the client, the Nayax Engine uses the [StaticQR Cancel](/reference/cortina/staticqr/cortina-staticqr-cancel) endpoint to cancel the Pre Authorization and the transaction.
</Warning>