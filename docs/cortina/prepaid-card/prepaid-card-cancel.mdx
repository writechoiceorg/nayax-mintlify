...
title: "Cancel"
metadata:
  og:description: "Cortina Prepaid uses the Cancel command to notify the integrator in order to initiate the cancelation process with the Cortina payment provider."
---

Cortina Prepaid uses the `Cancel` command to notify the integrator in order to initiate the cancelation process with the Cortina payment provider whenever one of the below scenarios takes place:
