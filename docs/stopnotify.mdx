...
title: "StopNotify"
metadata:
  og:description: "Enable StopNotify This feature must be enabled in Nayax Core during the integration. The StopNotify endpoint enables Spark to determine whether a."
---
...
Use the `StopNotify` endpoint to determine whether a card presented at the payment terminal while idle can stop an ongoing session. This applies primarily to Pre-Authorization flows, where the same card must be presented again to terminate the session and proceed to settlement.

Nayax will send the hashed card value and the last four digits captured earlier in the `/TransactionCallback` and `/TransactionNotify` along with terminal identifiers. You need to validate whether this card matches an ongoing session on that terminal. A matching card should result in an Approved response; otherwise, return a Declined verdict.

This feature must be enabled via Nayax Core. If you do not respond within 5 seconds, the flow fails from the consumer's perspective.

## Request

Send a POST request to the Spark `/spark/StopNotify` endpoint with the following body:
