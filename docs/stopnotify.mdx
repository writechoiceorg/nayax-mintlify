...
Nayax sends the hashed card value, last four digits (from `/TransactionCallback` and `/TransactionNotify`), and terminal identifiers. Validate whether this card matches an ongoing session on that terminal. Return an **Approved** response for a matching card. Otherwise, return a **Declined** verdict.

Enable this feature in Nayax Core. If you do not respond within 5 seconds, the flow fails from the consumer's perspective.
