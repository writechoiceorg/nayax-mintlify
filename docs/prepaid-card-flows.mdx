...
2. The Cortina API then initiates a service session using the [Start Session](/reference/cortina/cortina/post-start-session) endpoint and sends a [Prepaid Sale](/reference/post_cortina-prepaid-sale) request to the Integrator. This request includes all the necessary details to authorize the payment for the product or service.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the[ Prepaid Void](/reference/post_cortina-prepaid-void) endpoind to void the payment request.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a `992` response code is be received,  the Nayax Engine uses the [Prepaid Void](/reference/post_cortina-prepaid-void) endpoind to void the request.
...
4. The Nayax Engine performs an [Authorization Request](/reference/post_cortina-prepaid-authorization) to the Integrator using Cortina APÌ. This request checks if the prepaid card has sufficient funds for the transaction.
...
8. The Nayax Engine forwards the Settlement Request to the integrator using Cortina's API [Prepaid Card Settlement](/reference/post_cortina-prepaid-settlement) endpoint, which includes the final transaction amount to be settled.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [ Prepaid Cancel](/reference/post_cortina-prepaid-cancel) endpoint to cancel the Pre Authorization and the transaction.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived,   the Nayax Engine uses the [Prepaid Void](/reference/post_cortina-prepaid-void) endpoind to void the payment request. To indicate that the Void is sent due to timeout, the flag `isGatewayTimeout` will be sent as `True`.
</Warning>
