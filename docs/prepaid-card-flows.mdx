...
6. Once the service is completed or the product has been dispensed, the Nayax Device sends a[ Sale End](https://developerhub.nayax.com/reference/post_cortina-saleendnotification) request in the Cortina API. This indicates the completion of the transaction and that the customer has received the service or product.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the[ Prepaid Void](https://developerhub.nayax.com/reference/post_cortina-prepaid-void) endpoind to void the payment request.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a `992` response code is be received,  the Nayax Engine uses the [Prepaid Void](https://developerhub.nayax.com/reference/post_cortina-prepaid-void) endpoind to void the request.
...
4. The Nayax Engine performs an [Authorization Request](https://developerhub.nayax.com/reference/post_cortina-prepaid-authorization) to the Integrator using Cortina APÌ. This request checks if the prepaid card has sufficient funds for the transaction.
...
8. The Nayax Engine forwards the Settlement Request to the integrator using Cortina's API [Prepaid Card Settlement](https://developerhub.nayax.com/reference/post_cortina-prepaid-settlement) endpoint, which includes the final transaction amount to be settled.
...
In case the product fails to be dispensed to client, the Nayax Engine uses the [ Prepaid Cancel](https://developerhub.nayax.com/reference/post_cortina-prepaid-cancel) endpoint to cancel the Pre Authorization and the transaction.
</Warning>

<Warning>
**Timeout Void**

In case that no Sale response will be recived within the timeout period, or that a 992 response code will be recived,   the Nayax Engine uses the [Prepaid Void](https://developerhub.nayax.com/reference/post_cortina-prepaid-void) endpoind to void the payment request. To indicate that the Void is sent due to timeout, the flag `isGatewayTimeout` will be sent as `True`.
</Warning>
