...
2. The Cortina API then initiates a service session using the [Start Session](/reference/post_cortina-startsession) endpoint and sends a [Prepaid Sale](/reference/post_cortina-prepaid-sale) request to the Integrator. This request includes all the necessary details to authorize the payment for the product or service.
...
6. Once the service is completed or the product has been dispensed, the Nayax Device sends a[ Sale End](/reference/post_cortina-saleendnotification) request in the Cortina API. This indicates the completion of the transaction and that the customer has received the service or product.
