...
metadata:
  og:description: "The refund process begins by submitting a request with details such as the refund amount and reason. This notifies the customer that the refund is."
---
...
You can only request refunds for transactions made by credit card. This option is not available for prepaid cards.
</Warning>

In Nayax Core, you can request refunds through the **Dynamic Transactions Monitor**. Follow the steps below to do so.
...
You can perform this process programmatically through the Lynx API. The following endpoint is used to process the refund.

<EndpointCard
  title="Request a Payment Refund"
  href="/reference/lynx/payment/request-payment-refund"
  method="post"
/>
...
To start the request, use the [Request a Payment Refund](/reference/lynx/payment/request-payment-refund)  endpoint to request a refund. Providing the attributes regarding the rebate in the body parameters of the request, as exemplified below:
...
The `TransactionId `and `SiteId ` can be retrieved in the Lynx API using the [ Get Last Sales for Machine ](/reference/lynx/machines/get-last-sales-for-machine) endpoint to retrieve the last sales using the `MachineID` and the response object should have the necessary data.

## Request Status

Once you send the request, the system sends a notification email to the email address you entered in the request parameters.
