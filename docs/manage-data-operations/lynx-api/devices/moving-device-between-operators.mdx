...
metadata:
  og:description: "Lynx API allows you to reassign devices from one operator to another within the hierarchy in your Nayax system. This is particularly useful for."
---
...
Use the [Move Devices](/reference/lynx/devices/move-devices-to-actor-id) endpoint to assign or move devices to the specified `actorId`. The request body should include an array of device identifiers, such as serial numbers. See the example request in the code block below:
...
<Note>
**Empty response means no devices matched**

If the returned array is empty (`[]`), no devices matching the provided serial numbers were found on your account. The endpoint returns `200 OK` in both cases — check the response array to confirm devices were actually moved.
</Note>
