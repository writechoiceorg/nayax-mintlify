...
Access tokens are managed through the Nayax Back Office (Nayax Core). Follow these steps to reach the **User Tokens** area, then create a new token or retrieve an existing one:
...
To reuse a token you created earlier, click the **Show Token** button next to it in the **User Tokens** section, then copy the token for use in API calls.
