...
Use the [Update Specific Machine Attribute](/reference/lynx/machine-attribute/update-specific-machine-attribute) endpoint to set device attribute `144` on the machine. Provide the `MachineID` and the attribute's `DeviceAttributeID` as path parameters, and the attribute value as a body parameter, as shown below:
...
| `DeviceAttributeValue` | `string` | The value assigned to the attribute. Use `"1"` to trigger the restart.   |
