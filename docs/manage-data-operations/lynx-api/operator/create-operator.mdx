...
metadata:
  og:description: "In Nayax Core, you can create and manage operators using the Operator menu. To create a new Operator, follow the steps below: 1. Go to Administration."
---
...
<Frame><img src="/images/docs/create-operator/image1.png" /></Frame>
...
<EndpointCard
  ...
  href="/reference/lynx/actors/create-new-actor"
  method="post"
/>
...
The [Create New Actor ](/reference/lynx/actors/create-new-actor)endpoint allows for registering a new operator in the Nayax system. See the example request in the code block below:
...
A v2 endpoint is also available at `POST /operational/v2/actors/{ParentActorID}`. The v2 request body differs from v1 in the following ways:

<Warning>
**v2 Field Differences**

| v1 field | v2 equivalent | Notes |
|----------|--------------|-------|
| `ActorStatus` | `StatusID` | Field was renamed in v2 |
| *(not present)* | `ActorBillingPlanID` | **Required** in v2. An invalid or missing value returns `400 Bad Request`. Contact your Nayax distributor to obtain valid billing plan IDs for your account. |
| *(not present)* | `ActorBillingGateways` | Array of billing gateway objects. |

Use the v1 endpoint unless you specifically need billing plan or gateway configuration at creation time.
</Warning>
...
<Frame><img src="/images/docs/create-operator/image2.png" /></Frame>
