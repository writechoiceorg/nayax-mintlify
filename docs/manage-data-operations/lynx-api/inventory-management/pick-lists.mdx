...
Use the [Create Pick List](/reference/lynx/machine-inventory/generate-pick-list) endpoint to create a list for a specified machine based on its current stock levels and sales data.
...
<Note>
**Empty response body is expected**

A successful `POST` to this endpoint returns `200 OK` with no response body. To confirm the pick list was created, follow up with `GET /v1/machines/{MachineID}/pickList`.
</Note>

<Note>
**curl requires an explicit Content-Length header**

When using curl with an empty body, add `-H "Content-Length: 0"` to avoid a `411 Length Required` error:
