...
You need a Nayax Core account to create the refresh token. A Nayax admin
must assign its scopes and claim type before you can use it, so requesting a
token involves waiting on that approval rather than a fully self-service flow.
