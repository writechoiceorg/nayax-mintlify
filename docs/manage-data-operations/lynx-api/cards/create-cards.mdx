...
title: "Create Cards"
metadata:
  og:description: "Cards serve as versatile solutions for various business needs, such as facilitating payments, issuing refunds, providing discounts, or granting."
---
...
* **Prepaid Card**: Preloaded with funds, prepaid cards are ideal for controlled spending. They are commonly used for gift cards, employee stipends, or consumer transactions and offer convenience and security.
...
In this guide, we will be showing an API request creating a prepaid card, but all of them can be created in the same way.
...
<EndpointCard
  ...
  href="/reference/lynx/cards/create-virtual-card"
  method="post"
/>

<EndpointCard
  title="V2: Create New Card"
  href="/reference/lynx/cards/create-new-card"
  method="post"
/>
</Columns>
...
* `CountryID`: Nayax internal country ID. Use `GET /v1/lookups/countries` to retrieve the correct value for your region. Note: card endpoints use Nayax internal IDs, not ISO numeric codes.
...
To create the card, send a **POST** request to the[ Create New Card](/reference/lynx/cards/create-new-card) endpoint, filling in the card details in the body parameters. See the example in the code block below:
...
<Warning>
**Prepaid cards require additional credit limit fields**

When creating a prepaid card (`CardTypeID: 33`), the `CardCreditLimits` object must include three additional fields not required for other card types:

```json
"CardCreditLimits": {
  "AmountDailyLimit": 100,
  "AmountMonthlyLimit": 1000,
  "CreditAmountDailyLimit": 100,
  "CreditAmountMonthlyLimit": 1000,
  "CreditAmountMonthlyReload": 0
}
```

Omitting any of these fields returns `400 Field CreditAmountDailyLimit is required` (or the next missing field).
</Warning>

<Warning>
**GroupLocationLimits behavior**

When **creating** a card, set `GroupLocationLimits` to `null`. Sending an empty array `[]` causes a `500` error.

When **updating** a card, the array must exactly match the card's current groups. Fetch the card first with `GET /v1/cards` and pass the same `GroupLocationLimits` array back. Adding or removing groups via this field is not supported.
</Warning>

<Warning>
**Revalue endpoints require RevalueCashBit set at creation**

The card revalue endpoints (`GET /v1/cards/{id}/revalue` and `POST /v1/cards/{id}/revalue/add`) only work on cards where `CardCreditAttributes.RevalueCashBit` is `true`. This must be set when the card is created — it cannot be enabled after the fact. Cards created without `RevalueCashBit: true` return `400 "This Card is not defined as Revalue"` on all revalue endpoint calls.
</Warning>
...
Use `PUT /v1/cards/{CardID}` to update an existing card's details.

<Note>
**CardPhysicalType is required when updating**

`PUT /v1/cards/{CardID}` requires `CardPhysicalType` in the request body even when you are not changing it. Omitting this field returns `400 "The field CardPhysicalType is not valid"`. Fetch the card first with `GET /v1/cards` to read the current value, then include it in your update request.
</Note>
