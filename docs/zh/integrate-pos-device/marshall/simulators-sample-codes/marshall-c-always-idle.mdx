...
title: C SDK 始终空闲模式
metadata:
  og:description: SDK 还可以将外围设备配置为保持空闲状态,这意味着交易流程将从外围设备端开始,通过发送 Vend。
---

SDK 还可以将外围设备配置为保持空闲状态,这意味着交易流程将从外围设备端开始,通过发送带有相关商品信息的“Vend Request”(出货请求)来启动(而不是由外围设备在商品选择后启用设备的读卡器,然后设备针对消费者出示的卡片响应“Begin Session”(开始会话),之后外围设备才发送“Vend Request”)。

本页将更详细地指导您在 C SDK 中实现始终空闲状态。为了更好地说明此流程,代码实现分为以下几个步骤:
...
实现用于调试、处理事件和处理交易的辅助函数。以下各节将提供更多详细信息。

### 调试输出函数

此函数确定调试信息的输出流,支持不同的平台:

- Linux/X8：使用 `printf()`。
- STM32 微控制器：使用 `drv_usart_tx_data()`。
...
由于您希望使 status 保持为 always idle，请将 `config.always_idle` 设置为 `__true`。
...
现在，您可以通过调用之前创建的处理程序来初始化菜单。请参见以下代码。
