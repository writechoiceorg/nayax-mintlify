...
title: C SDK 多会话
metadata:
  og:description: 多会话支付允许独立处理多笔交易，从而确保在自助服务环境中获得无缝的用户体验。
---

多会话支付允许独立处理多笔交易，从而确保在自助服务环境中获得无缝的用户体验。

本页将指导您更详细地使用 C SDK 实现多会话支付。为了更好地解释此过程，代码实现分为以下步骤：
...
下方代码定义了用于日志记录/调试的模块名称宏，并设置了用于存储配置和会话相关数据的全局变量。
...
- `config`：存储自动售货机配置。
- `session_products`：表示两个商品的 array，每个会话一个。
  - 第一个是针对会话 0。
  - 第二个是针对第 1 场会议的。
- `sessions`：表示两个活动会话的 array。
- `current_session`：跟踪活动会话。
- `menu`：存储菜单相关设置。
- `com_id`：表示通信 ID。
...
该函数将始终返回布尔值 `__false`，因为未确定成功/失败。

### 多会话销售请求处理程序

此函数管理会话及其包含的项目，然后向 Nayax 设备发送售货请求。请参见以下代码块：
...
它初始化 `session_products` 0 和 1，每个都具有不同的商品代码和价格。每一个都将是一个单独的会话（交易）。
当然，这是一个演示应用，这意味着您可以（也应该）管理自己的商品和价格，以匹配您的外围设备。

### 多会话出货事件处理器

此功能处理多会话的销售事件并记录相关信息。
...
此函数用于初始化 VMC 配置并注册事件处理程序。请参见下方代码：
...
static void vmc_init(void)
{
	...
	config.price_not_final_support = __true;
...
由于这是多会话支付，` config.multi_session_support` 被设置为`__true`。
...
现在，您可以初始化链接之前创建的处理程序的菜单。请参阅下面的代码。
