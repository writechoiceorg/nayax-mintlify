...
title: C SDK 多供应商
metadata:
  og:description: Nayax 中的多次贩售流程允许消费者在单个支付会话中购买多件商品，并使外围设备能够。
---

Nayax 中的多次贩售流程允许消费者在单个支付会话中购买多件商品，并使外围设备能够区分这些产品。这在自动售货机和自助服务终端中非常有用，因为客户可能希望在完成支付前选择多种产品。

本页面将更详细地指导您在 C SDK 中实现多次贩售会话。为了更好地解释此过程，代码实现分为以下几个步骤：
...
- `config`：存储自动售货机配置。
- `session_products`：持有两个不同售卖商品的 array。
- `session`：存储会话详情。
- `menu`：存储菜单相关设置。
- `com_id`：表示通信 ID。
...
它初始化 `session_products` 0 和 1，每个都具有不同的商品代码和价格，并将它们添加到 “session” object 中。
...
由于这是多重出货会话，` config.multi_vend_support` 被设置为`__true`。
...
现在您可以通过链接之前创建的处理程序来初始化菜单。请参见以下代码。
