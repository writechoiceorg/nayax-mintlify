...
* **PUT Method**: With this option you can use the same endpoints for creating or retrieving a cards data, but using a **PUT** request with the new amount of credit. For example you can use the [Update Card by ID](/reference/update-card-by-id) endpoint with the `CardID` in the URL and the updated credit amount in the body parameters.
...
To add credit to a card you can use the **POST** method on the [Add Credit to a Card](/reference/add-card-credit) endpoint. Sending the card's Unique Identifier in the URL. See the example query below:
