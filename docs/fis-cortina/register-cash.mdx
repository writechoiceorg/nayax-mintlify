...
Your server must return a valid `fiscalResponse` object. At minimum:
...
```json
{
  "fiscalResponse": {
    ...
    "FiscalInfo": {
      "FiscalRrn": "your-fiscal-rrn-here",
      "FiscalId": 1234567,
...
- `Verdict` is **mandatory**. A missing or invalid `Verdict` causes the action to fail with a parsing error.
- `FiscalRrn` is **mandatory** in the registration response. Nayax echoes it in any future Void requests for this transaction.
- `FiscalReceiptUrl` is optional. If provided, the device presents it to the consumer as a QR code on the screen.

See [Request & Response Reference](/docs/fis-cortina/response-reference) for the full list of accepted `ResponseCode` values.
