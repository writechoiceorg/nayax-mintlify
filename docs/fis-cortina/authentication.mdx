...
description: "How to implement the StartSession challenge-response to authenticate each fiscal session."
---
...
Nayax and each integrator share a unique [**Secret Token**](/docs/fis-cortina/glossary): a 66-character string provided by Nayax at kickoff. The last 32 characters of this token serve as the AES-256 encryption key.
...
<Note>
**Validate Transaction IDs on your end.** When Nayax sends a `Register`, `Void`, or `Refund` request, verify that the `BasicInfo.TransactionId` was issued by your server in a prior `StartSession` call and has not expired. Transaction IDs should remain valid for no longer than **10 minutes**.
</Note>
...
<Steps>
  ...
  <Step title="Nayax decrypts and validates">
    ...
    If validation fails, the session is rejected.
  </Step>

  <Step title="Transaction ID used in subsequent calls">
    Nayax uses the decrypted Transaction ID in all subsequent [`Register`](/docs/fis-cortina/register), [`Void`](/docs/fis-cortina/void), and `Refund` requests for the session. It appears in the `BasicInfo.TransactionId` field.

    Your server must validate this ID against your own records on every incoming request.
  </Step>
</Steps>
...
Use the [devglan.com AES decryption tool](https://www.devglan.com/online-tools/aes-encryption-decryption) to confirm your output is correct before testing with Nayax:
...
Decrypt the output and confirm it matches the plaintext you built in [Step 3](#build-the-plaintext).
