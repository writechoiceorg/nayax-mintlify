...
<Warning>
  Nayax encourages merchants to choose the [Payment with Pre-Authorization](/docs/payment-with-authorization) approach since it's safer in cases of communication problem.
</Warning>
...
1. The host application initiates a payment transaction by calling the [`PayTransaction`](/docs/payment-1#paytransaction) method. The total transaction amount, currency, and other parameters are specified to EMV Core upfront.
...
8. EMV Core receives the transaction result from the server and triggers the [`TransactionComplete`](/docs/event-callbacks#transactioncomplete) event, sending the transaction's data.
