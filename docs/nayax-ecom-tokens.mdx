...
The `/payment` endpoint is used for recurring payments. It should be used for calls to recurring payments and requires a card token to be added to the request. These calls are performed from the server only.
