...
The Marshall implementation of [ExternalSettlement](/reference/post_externalsettlement) separates the authorization and settlement phases of a transaction.
...
For this flow to function, the [ExternalSettlement](/reference/post_externalsettlement) setting must be enabled by Nayax.
</Note>
...
* The only way to settle a transaction is via the [ExternalSettlement](/reference/post_externalsettlement) command.
* For Multi-session flows, the "Close Session" command is no longer relevant; use [ExternalSettlement](/reference/post_externalsettlement) instead.
...
21. The 3rd Party Server sends an [ExternalSettlement](/reference/post_externalsettlement) request. It must contain: `NayaxTransactionId`, `SiteId` (information retrieved from the transaction's Transfer Data's command) and be sent within 48 hours of the authorization of the consumer's card.
...
25. Nayax's servers send an [ExternalSettlement](/reference/post_externalsettlement) response to the 3rd-party server.
...
26. The 3rd Party Server sends an [ExternalCancel](/reference/post_externalcancel) request. It must contain: `NayaxTransactionId`, `SiteId` (information retrieved from the transaction's Transfer Data's command) and be sent within 48 hours of the authorization of the consumer's card.
...
29. Nayax's servers send an [ExternalCancel](/reference/post_externalcancel) response to the 3rd Party Server.
