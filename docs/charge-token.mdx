...
Requests are performed to the[ `/charge-token`](/reference/post_charge-token) endpoint.
...
| `tokenModel`                | basicInfo      | Int32          | The specific tokenization scheme used. **Token Type Behavior**:  A token's type (subscription/COF/top-up) is set during its initial creation and stored in our system. If a subsequent [Charge Token](/reference/post_charge-token)   request includes a different token type parameter, it is ignored. The system always uses the original token type from the initialization request, regardless of what is provided in later charge requests. | Yes      |
