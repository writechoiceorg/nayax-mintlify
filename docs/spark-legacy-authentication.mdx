...
1. Prepare the JSON body with all the necessary fields (e.g., `TerminalId`, `Cipher`, etc.). Here’s a simplified example for the [StartAuthentication](/reference/post_startauthentication) endpoint:
...
For this example, the [`/StartAuthentication`](/reference/post_startauthentication)  method was used, but the process is relevant for any Spark endpoint request and response.
...
<Card
  title="API Methods"
  href="/v1.0/docs/api-methods-spark"
  arrow
/>
</Columns>