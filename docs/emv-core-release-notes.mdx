...
<Accordion title="Improvements" icon="fa-rocket">
...
<Accordion title="Fixes" icon="fa-wrench">
  ### **Event Delivery and Reader Stability (#7443, #7497)**

  Fixed an intermittent issue where [`ReaderMessageEvent`](https://developerhub.nayax.com/docs/reader-messages) would stop arriving at the host application. Additionally, we resolved a connectivity bug where the EMV Core was unable to reconnect to a serial reader following a POS firmware update.
...
<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ...
  ***
  ...
  There is a known intermittent issue where the EMV Core may fail to send the [`ReaderMessageEvent`](https://developerhub.nayax.com/docs/reader-messages) with index 36 (“Authorizing”). Applications should handle the transaction flow even if this specific status update is missing.
</Accordion>
...
<Accordion title="Fixes" icon="fa-wrench">
  ...
  Resolved logic issues in `PRE_AUTH_AMOUNT` and fixed a hang where [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) would wait for a timeout if the reader was in boot. We also corrected a mutex timing bug that caused false-positive `SAT_Mutex_Acquire` timeout errors on some systems.

  ### **Connectivity and Firmware Fixes (#7157, #7165, #6813)**

  Fixed connectivity issues for the UNO-mini TTL after a reset and resolved recognition errors for Mifare PLUS ISO 14443-4 as closed loop. We also addressed a failure in [`GetCardToken`](https://developerhub.nayax.com/docs/payment-1) for specific Visa and Amex cards.
...
<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ...
  ***
  ...
  The SDK does not currently trigger [`SystemStatusChanged`](https://developerhub.nayax.com/docs/event-callbacks) for network changes and may report a `Ready` status while the reader is still booting. Furthermore, [`CancelTransaction`](https://developerhub.nayax.com/docs/payment-1) will return "Cannot Cancel" if a transaction is already Authorizing or Completed.

  ***

  ### **Hardware and Resource Usage**

  On UNO-mini hardware, the Left LED may stay illuminated after a poll timeout. Additionally, when a reader is unavailable, [`GetStatus`](https://developerhub.nayax.com/docs/remote-management) calls occur at a high frequency (every 1ms), which may increase resource usage.
...
<Accordion title="Improvements" icon="fa-rocket">
  ...
  The [`GetCardToken`](https://developerhub.nayax.com/docs/payment-1) command has been updated to return the Mifare UID. This improvement allows for more precise identification of Mifare-based cards during the tokenization process, providing consistent card data across different card types.
...
<Accordion title="Fixes" icon="fa-wrench">
  ...
  Fixed an issue where the UID for closed loop cards was missing from the [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) event. The SDK now correctly and consistently returns the UID upon transaction completion.
...
<Accordion title="Fixes" icon="fa-wrench">
  ...
  Fixed a critical issue where the EMV Core would crash when no internet connection was detected. We also updated the [`Void`](https://developerhub.nayax.com/docs/payment-1) command logic: calling [`Void`](https://developerhub.nayax.com/docs/payment-1) after a [`Pre-Authorize`](https://developerhub.nayax.com/docs/payment-1) or `Pay` no longer incorrectly triggers a [`Cancel Transaction`](https://developerhub.nayax.com/docs/payment-1), allowing a new transaction to start independently.
...
<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ...
  ***
  ...
  The current UNO-mini firmware does not support [`ReaderMessageEvent`](https://developerhub.nayax.com/docs/reader-messages), which may limit real-time visual or text feedback during certain operations on this hardware.

  ***

  ### **GetCardToken and Polling Limitations (#6978)**

  The newly added [`GetCardToken`](https://developerhub.nayax.com/docs/payment-1) command does not yet support Continuous polling mode. The process will terminate after a single attempt or timeout.
...
<Accordion title="Improvements" icon="fa-rocket">
  ...
  ***
  ...
  We have added the [`GetAttribute()`](https://developerhub.nayax.com/docs/remote-management) command to the API, allowing host applications to retrieve configuration attributes directly from the SDK. This ensures the host can always verify the active configuration state in real-time.
</Accordion>
...
<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ...
  ***
  ...
  The EMV Core does not currently trigger the [`SystemStatusChanged`](https://developerhub.nayax.com/docs/event-callbacks) callback for network connectivity changes. Developers should implement independent monitoring logic if the application needs to react to network state changes.

  ***

  ### **Transaction Completion Event Delivery (#6582) (Linux Only)**

  There is an intermittent issue where the [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) event is not always successfully transmitted to the host application.
  ...
  **Workaround:** Wait for the [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) event or use `GetStatus()` to verify terminal readiness; it will return `PaymentTransaction` if the reader is still polling.
</Accordion>
...
<Accordion title="Fixes" icon="fa-wrench">
  ...
  Confirmed that calling [`PreAuthorize`](https://developerhub.nayax.com/docs/payment-1) while a previous [`PreAuthorize`](https://developerhub.nayax.com/docs/payment-1) command is still active (polling) is intentionally rejected.

  **Guidance:** Developers should use the [`GetStatus()`](https://developerhub.nayax.com/docs/remote-management) command to verify if the terminal is ready. If the device is currently polling for a card, [`GetStatus()`](https://developerhub.nayax.com/docs/remote-management) will return the `PaymentTransaction` status.
</Accordion>

<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ### **Transaction Completion Event Delivery (#6582)**

  There remains an intermittent issue where the [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) event is not always successfully sent to the host application.
</Accordion>
...
<Accordion title="Known Issues" icon="fa-exclamation-triangle">
  ...
  ***
  ...
  Initiating a [`PreAuthorize`](https://developerhub.nayax.com/docs/payment-1) command while a previous one is still active (and no card has been presented) will cause the command to fail. Users should ensure the current transaction is finalized or timed out before starting a new one.

  ***

  ### **Transaction Event Delivery (#6582)**

  The [`TransactionComplete`](https://developerhub.nayax.com/docs/event-callbacks) event is not always successfully transmitted to the host application, which may require the application to poll for status updates to confirm transaction outcomes.
</Accordion>
