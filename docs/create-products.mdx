...
<Frame>![](/images/docs/create-products/images1.png)</Frame>
...
You can add a new product to an operator's catalog using the [Create New Operator Product ](ref:create-operator-product) endpoint. See an example request in the code block below:
...
You can create multiple products for a single machine using the [Create Machine Products](ref:create-machine-products) endpoint. This allows you to add several products in one request, even if they belong to different product groups.
...
You can use the [Update Specific Product](ref:update-specific-product) endpoint to change a product's details by adding the product's `NayaxProductID` in the path params and the information to change in the body request. As in the example request below:
