...
The examples below install `nayax-lynx-inventory`. To install a different skill, swap in its name from the table above. The `skills` CLI works with any compatible agent and is the simplest way to start. If you use Claude Code, you can install skills as a plugin instead, which keeps them updated automatically.
...
With the `skills` CLI you can install more than one skill in a single command.

Install every Nayax skill, with no prompts:

```bash
npx skills add writechoiceorg/nayax-ai --all
```

Install a specific set by repeating the `--skill` flag:

```bash
npx skills add writechoiceorg/nayax-ai --skill nayax-lynx-inventory --skill nayax-lynx-refunds
```

Run the command without `--skill` to choose from the full list interactively:
