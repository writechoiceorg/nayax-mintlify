...
<EndpointCard
  ...
  href="/reference/lynx/payment/request-a-payment-refund"
  method="post"
/>
...
The `TransactionId `and `SiteId ` can be retrieved in the Lynx API using the [ Get Last Sales for Machine ](/reference/lynx/machines/get-last-sales-for-machine-by-machineid) endpoint to retrieve the last sales using the `MachineID` and the response object should have the necessary data.
